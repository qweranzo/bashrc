...
# TO DO: set MAX LIMIT FOR jpgmin pngmin pngif jpgif !!!!!!!!!!
# TO DO: --optimize -O2 Also uses transparency! to shrink the file further. Add transparency flag
...
# Что делает: 
	...
	# сжимает gif и уменьшает разрешение в два раза 
	# переименовывает гифы по имени папки, в которой они находятся
	# вытаскивает эти гифы в родительский каталог (где сами папки лежат)
# Как пользоваться: jpgif [lossy] [colors] [delay] [loop] [scale] - параметры опционально, 
	# по-умолчанию: 
		# lossy=20 (ПРИМ.: в отличие от jpgmin и pngmin, чем выше значение, тем сильнее компрессия, т.е. ниже качество)
...
# Пример: jpgif
# Пример 2: jpgif 74 
# Пример 3: jpgif 82 152 100 4 1 - склеить в гифы с компрессией 82, оставить 152 цвета, 1 секунда на кадр, с 4-мя повтороами, не меняя масштаб
...
function clearexif () {
	...
	if [ -z $1 ] 
	then
		echo "No args. Usage: \"clearexif filename.extension\" or \"clearexif all\" ";
		return 0;
	else 
...
# WSL: batch open files via Windows  
function winstart () {
